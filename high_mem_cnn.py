...
def build_model(data, labels, mode):
    ...
    conv1 = tf.layers.conv2d(inputs=data, filters=8, kernel_size=5, padding="same", strides=[1,1,1,1], activation=tf.nn.relu, name="conv1")
    print ("CONV " + str(conv1))
    tf.Print(conv1, [conv1], message="CONV1 ")
    conv2 = tf.layers.conv2d(inputs=conv1, filters=8, kernel_size=5, padding="same", strides=[1,1,1,1], activation=tf.nn.relu, name="conv2")
    ...
    flat_pool = tf.reshape(pool2, [BATCH_SIZE, 155184])
